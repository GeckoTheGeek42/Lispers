...
macro_rules! lisp_fold_func(
	(|$ee:ident, $args:ident| -> { $temp:ident = $start:expr + |$acc:ident, $elem:ident| -> $fun:expr; $wrap:expr } ) => 
		( lisp_func!(|$ee, $args| -> { 
			let $temp = args.iter().fold( $start, |$acc, $elem| { $fun } );
			$wrap
		}) );
	(|$ee:ident, $args:ident| -> { $temp:ident = |$acc:ident, $elem:ident| -> $fun:expr; $wrap:expr } ) =>
		( lisp_func!(|$ee, $args| -> { 
...
pub fn init_functs(functs: &mut HashMap<&str, LispFunc>) {
	...
	functs.insert("-", math_fold_funct!(|a, n| -> a - n));
	functs.insert("/", math_fold_funct!(|a, n| -> a / n));	
	functs.insert("concat", lisp_fold_func!(|ee, args| -> ));
}